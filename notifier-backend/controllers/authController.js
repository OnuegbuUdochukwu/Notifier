...
module.exports.signUp = async (req, res)=>{
    ...
    try{
        ...
        if (userExists) res.send(400).json({message: "that user alredy exists"})

        const user = await User.create({username, email, password})
        createToken(res, user._id)
        
        res.status(201).json({message: "user succesfully created", user: {username, email}})
...
module.exports.login = async (req, res)=>{
    ...
    try{
        ...
        createToken(res, {userId: user._id})
